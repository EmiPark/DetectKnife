...
const char * SCTD = "C:\\CodingWorkspace\\Resource\\SCTemplateData.txt";
...
double ** calSC(CvPoint *points,CvMemStorage *storage,CvSeq *contour)
{
	...
	*/
	...
	for (size_t i = 0; i < onetourlength; i++)
	{
		if (i % 4 == 0) 
		{
		mypoints[i / 4].x = points[i].x;
		mypoints[i / 4].y = points[i].y;
		//cout <<onetourlength<<"	 "<<nPointNeed<<"  "<< onetourlength / nPointNeed << endl;
		//nPointNeed++;
		//	center.x = points[i].x;
		//	center.y = points[i].y;
		//	cvSeqPush(allpointsSeq,&center);
		//	cvCircle(dst, center, 1, Scalar(255, 255, 255), 1, 8, 0);
		//cout << "第"  << "个轮廓的" << "第" << i /4<< "个点的坐标：X：" << points[i].x << "  Y:" << points[i].y << endl;
		}
	else continue;
	}
	cout << nPointNeed << endl;
	//nPointNeed = onetourlength / 4;
	*/
	/*
	for (size_t i = 0; i < nPointNeed; i++)
	{
		srand(time(NULL));
		index = rand() % (onetourlength - i) + i;
		mypoints[i].x = points[index].x;
		mypoints[i].y=points[index].y;
	}
	*/
	...
	*/
	...
	for (size_t i = 0; i < nPointNeed; i++)
	{
		for (size_t j = 0; j < nPointNeed; j++)
		{
			//cout <<i<<"点坐标是:" <<treatPoints[i].x << "," << treatPoints[i].y << endl;
			//cout << j << "点坐标是:" << treatPoints[j].x << "," << treatPoints[j].y << endl;
			//cout << "第" << i << "个点与第" << j << "个点的距离是" << r_array[i][j] << endl;
			//cout << "第" << i << "个点与第" << j << "个点的角度是" << atan2(treatPoints[j].y - treatPoints[i].y, treatPoints[j].x - treatPoints[i].x)*180/PI<< endl;		 //看实际角度可 * 180 / PI
			//cout << "第" << i << "个点与第" << j << "个点量化后的角度是" << theta_array[i][j] << endl;
		}
	}
	*/
	...
	//将上下文信息放入SC数组中
		...
		{
		SC = new double*[nbins_r];
		for (size_t k = 0; k < nbins_r; k++)
		{
		SC[k] = new double[nbins_theta];
		}
	...
	ifstream input;
	string s;
	input.open(SCTD, ios::_Nocreate);
	if (input.is_open())
	{
		input.seekg(-(int)sizeof(int) / 2, ios::end);
		//cout << input.get()-'0' << endl;
		nLast = input.get() - '0';
	}
	input.close();
	//将SC数据存入txt文件中
	if ((contour->h_next == NULL) &&(contour->h_prev == NULL)&&(tempNum<nTemp)&&(nLast!=-3))		//轮廓没有前后序列，证明是模板序列，nLast用于探测是否结尾有-3
	{
		int row = nPointNeed;		//行数
		int col = nbins_theta*nbins_r;   //列数
		//ifstream input("C:\\CodingWorkspace\\Resource\\SCTemplateData.txt");
		//ofstream output("C:\\CodingWorkspace\\Resource\\SCTemplateData.txt");	//打开要写入的文件，如果该文件不存在，则自动新建
		output.open(SCTD, ios::app);			//在文件后追加内容，如果文件不存在则自动新建
		//D2Array out(row, D1Array(col, 0));
		...
		cout << "写入一个模板" << endl;
		output.close();
		tempNum++;				//控制模板数据只追加18次
	}
	//input.close();
	
	if (tempNum == nTemp)
	{
		output.open(SCTD, ios::app);
		output << -2 ;
		output.close();
		tempNum++;
	}
	*/
	...
	for (size_t i = 0; i < nPointNeed; i++)
	{
		for (size_t j = 0; j < nbins_r*nbins_theta; j++)
		{
			cout << "第" << i + 1 << "个元素的" << j + 1 << "个上下文是" << SC[i][j] << endl;
		}
	}
	*/
...
/*
以下代码为传统方法计算两个SC的代价矩阵（不从txt中提取数据）
*/
...
/*
以下代码为新方法计算一个轮廓SC与所有模板的代价矩阵并调用匈牙利算法直接求出最小最优值（从txt中提取数据）
*/
double calMinOpt(const char * filename, double **SC)
{
	...
	tempSC = new double*[nPointNeed];
	for (size_t i = 0; i < nbins_r*nbins_theta; i++)
	{
		tempSC[i] = new double[nbins_r*nbins_theta];
	}
	...
	for (size_t k = 0; k < nTemp; k++)
	{
		for (size_t i = 0; i < nPointNeed; i++)
		{
			for (size_t j = 0; j < nbins_r*nbins_theta; j++)
			{
				r = fscanf(f, "%lf", &d);		//fscanf成功时返回参数的个数
				if ((1 == r) && (d != -2))
				{
					n++;
					tempSC[i][j]= d;
		...
		if (minOpt>hungarian(HistCost(tempSC, SC, nPointNeed), dim, colsol, rowsol))
		{
			minOpt = hungarian(HistCost(tempSC, SC, nPointNeed), dim, colsol, rowsol);
		}
	}
	fclose(f);
	return minOpt;
...
double ** tempSC(IplImage * compareView,int contourSize)
{
	...
	for (size_t i = 0; i < compareView->height; i++)
	{
		for (size_t j = 0; j < compareView->width; j++)
		{
			...
			//Cdata[i][j]= static_cast<int>(((uchar *)compareView->imageData)[i*(compareView->widthStep / sizeof(uchar)) + j*compareView->nChannels + 0]);
		}
	}

	/*
	for (size_t i = 0; i < compareView1->height; i++)
	{
		for (size_t j = 0; j < compareView1->width; j++)
		{
			cvSetReal2D(compareView1, i, j, Ddata[i][j]);
		}
	}
